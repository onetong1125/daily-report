...
| 非标准错误（非 Error 实例） | ✅ | 未知异常，保守起见重试 |
| HTTP 4xx 非 429 (400, 401, 403...) | ❌ | 客户端错误，重试无意义 |
| 空内容（200 但无 content） | ❌ | 模型行为问题，重试大概率同样 |
| JSON 解析失败 | ❌ | 响应格式异常，重试大概率同样 |

`shouldRetry(error)` 对非标准错误的处理：如果 `error` 不是 `Error` 实例（如裸 string、null、自定义对象），视为网络错误，可重试。避免因未知 throwable 导致漏重试。
...
```
⚠️  LLM API 调用失败 (HTTP 502)，1s 后第 1/5 次重试...
⚠️  LLM API 调用失败 (网络错误)，2s 后第 2/5 次重试...
⚠️  LLM API 调用失败 (超时)，4s 后第 3/5 次重试...
✅ LLM API 第 3/5 次重试成功
```

全部失败时：
```
❌ LLM API 已重试 5/5 次，全部失败，回退到模板生成
```
